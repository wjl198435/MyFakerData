...
class SchedulerEngine(Thread):
    ...
    def __init__(self, client, name):
        ...
        self.connection = connect('/var/job/agent.db', check_same_thread = False)
        ...
        try:
            ...
            with self.mutex:
                try:
                    ...
                    result = self.create_job(schedule_item)
                    print('Update scheduled event result: {}'.format(result))
                    if result == True:
                        self.update_database_record(event['id'], event)
                        self.schedule_items[event['id']] = schedule_item
                except KeyError:
                    print('Old schedule with id = {} not found'.format(event['id']))
        except:
        ...
        with self.mutex:
            if remove_id in self.schedule_items:
                try:
                    ...
                    print('Key print removing scheduled item: {}'.format(remove_id))
        print('Remove id not found: {}'.format(remove_id))
        return False
    ...
    def create_job(self, schedule_item):
        ...
        print('Create job: {}'.format(schedule_item))
        try:
            with self.mutex:
    ...
    def run_scheduled_item(self, schedule_item):
        ...
        if not schedule_item:
            print('No scheduled item to run')
            return
        result = True
        ...
        if action_executed and 'http_push' in event:
            print('Scheduler making HTTP request')
            ...
            try:
                ...
                print('Scheduler HTTP request exception: {}'.format(ex))
                return None
            try:
                response = future.result(30)
                print('Scheduler HTTP response: {}'.format(response))
            except:
                pass
    ...
    def update_database_record(self, id, event):
        ...
        try:
            ...
            exception('print updating database')
            result = False
        print('Update database record result: {}'.format(result))
        return result
    ...
    def remove_database_record(self, id):
        ...
        try:
            ...
            result = False
        print('Remove database record result: {}'.format(result))
        return result

    def remove_all_database_records(self):
        """Remove all scheduled events from the database"""
        result = True
        print('Remove all database records')
        try:
            with self.mutex:
            ...
            result = False
        print('Remove all database records result: {}'.format(result))
        return result
